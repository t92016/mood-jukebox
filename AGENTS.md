...
- 後端代理：Vercel Serverless Functions（`api/`，藏 API Key 的防護層）
...
- 部署：GitHub repo → Vercel 自動部署

## 資料夾結構
```
├── api/                  ← 後端代理（recommend.js / youtube-search.js / lyrics.js）
...
├── vercel.json           ← Vercel 設定（/api/* → functions）
└── 專案的處理步驟/專案進度.md ← 詳細開發進度與踩坑記錄
```

## API Key 管理（重要）
- `GROQ_API_KEY`、`YOUTUBE_API_KEY`：只放 Vercel 環境變數（本機開發放 `.env`），**絕不進前端程式碼、絕不 commit**
...
## 常用指令
- `npm install` → 第一次安裝
- `npm run dev` → 本機啟動（vercel dev，含 functions）
